...
export async function storeGameImage(
  buffer: Buffer,
  sha256: string,
  brand: string,
  family: string,
  ...
  try {
    ...
    const storagePath = `games/${brandPath}/${familyPath}/${familyId}/${sha256}.jpg`;
    ...
    console.error(`Failed to store game image: ${error.message}`);
    return null;
  }
}

export async function checkGameImageExists(sha256: string): Promise<boolean> {
  try {
    const bucketName = getBucketName();
    const bucket = storageClient.bucket(bucketName);
