...
const MIN_DIMENSION = 200; // 200x200 minimum
...
export async function storeWatchImage(
  buffer: Buffer,
  brand: string,
  family: string,
  familyId: number,
  ...
  const storagePath = `watches/${safeBrand}/${safeFamily}/${familyId}/${sha256}.jpg`;
...
export async function getStoredImageUrl(storagePath: string): Promise<string> {
  const bucketName = getBucketName();
  return `https://storage.googleapis.com/${bucketName}/${storagePath}`;
}

export async function checkImageExists(sha256: string): Promise<boolean> {
  return false;
}
