import { db } from './db';
import { shoeFamilies, shoeImages, ShoeSeedReport } from '@shared/schema';
import { eq, sql, and, count, asc } from 'drizzle-orm';
import { downloadImage, validateImage, storeShoeImage } from './shoe-image-storage';
...
const JUNK_TITLE_FILTERS = [
  'kids', 'kid', 'toddler', 'infant', 'youth', 'gs', 'ps', 'td',
  'lot', 'bundle',
  'box only', 'replacement box', 'empty box',
  'laces', 'replacement laces',
  'for parts', 'damaged', 'defect', 'read description'
];
...
interface SeederStats {
  ...
  queuedFamilies: Array<{ brand: string; family: string }>;
...
async function searchEbayShoes(
  query: string,
  offset: number = 0,
  limit: number = 50
...
async function processShoeItem(
  ...
  const existingItem = await db.execute(sql`
    SELECT 1 FROM processed_shoe_items WHERE ebay_item_id = ${item.itemId} LIMIT 1
  `);
  ...
  for (const imageUrl of imageUrls) {
    try {
      ...
      if (!validation.valid || !validation.sha256 || !validation.buffer) {
        const reason = validation.error || 'validation_failed';
      ...
      const existingImage = await db
        .select()
        .from(shoeImages)
        .where(eq(shoeImages.sha256, validation.sha256))
        .limit(1);
      ...
      const stored = await storeShoeImage(
        validation.buffer,
        brand,
        family,
        familyId,
  ...
  await db.execute(sql`
    INSERT INTO processed_shoe_items (ebay_item_id, family_id, title, condition, image_count)
...
async function seedSingleShoeFamily(
  familyRecord: typeof shoeFamilies.$inferSelect,
  failureReasons: Map<string, number>
): Promise<FamilySeederResult> {
  const { brand, family, id: familyId } = familyRecord;
  
  const result: FamilySeederResult = {
    brand,
    family,
    imagesStored: 0,
    listingsScanned: familyRecord.listingsScanned,
    apiCalls: 0,
    duplicatesSkipped: 0,
    downloadFailed: 0,
    junkFiltered: 0,
  ...
  const existingCountResult = await db
    .select({ count: count() })
    .from(shoeImages)
    .where(eq(shoeImages.familyId, familyId));
  ...
  console.log(`\n${'─'.repeat(50)}`);
  console.log(`SEEDING SHOES: ${brand} ${family}`);
  ...
  const existingImages = await db
    .select({ sha256: shoeImages.sha256 })
    .from(shoeImages)
    .where(eq(shoeImages.familyId, familyId));
  ...
  for (const query of searchTerms) {
    ...
    while (currentImageCount < IMAGES_TARGET_PER_FAMILY && result.listingsScanned < MAX_LISTINGS_PER_FAMILY) {
      const { response: searchResult, apiCalled } = await searchEbayShoes(query, offset, 50);
      ...
      for (const item of searchResult.itemSummaries) {
        ...
        const itemResult = await processShoeItem(item, familyId, brand, family, existingSha256s, failureReasons);
      ...
      await db.update(shoeFamilies)
        .set({ listingsScanned: result.listingsScanned, updatedAt: new Date() })
        .where(eq(shoeFamilies.id, familyId));
...
export async function runShoeImageSeeder(): Promise<SeederStats> {
  ...
  console.log('SHOE IMAGE SEEDER v1.0 - FILL-FIRST MODE');
  console.log('='.repeat(60));
  ...
  while (true) {
    const activeFamilies = await db
      .select()
      .from(shoeFamilies)
      .where(eq(shoeFamilies.status, 'active'))
      .orderBy(asc(shoeFamilies.queueOrder));

    if (activeFamilies.length === 0) {
      const queuedFamilies = await db
        .select()
        .from(shoeFamilies)
        .where(eq(shoeFamilies.status, 'queued'))
        .orderBy(asc(shoeFamilies.queueOrder))
      ...
      for (const family of queuedFamilies) {
        await db.update(shoeFamilies)
          ...
          .where(eq(shoeFamilies.id, family.id));
      }
    ...
    const result = await seedSingleShoeFamily(family, stats.failureReasons);
    ...
    if (result.status === 'locked') {
      ...
      if (nextQueued.length > 0) {
        await db.update(shoeFamilies)
          .set({ status: 'active', updatedAt: new Date() })
          .where(eq(shoeFamilies.id, nextQueued[0].id));
  ...
  const remainingQueued = await db
    .select({ brand: shoeFamilies.brand, family: shoeFamilies.family })
    .from(shoeFamilies)
    .where(eq(shoeFamilies.status, 'queued'))
    .orderBy(asc(shoeFamilies.queueOrder));
...
function printSeederReport(stats: SeederStats): void {
  console.log('\n' + '='.repeat(60));
  console.log('SHOE SEEDER REPORT');
  console.log('='.repeat(60));
  ...
  console.log('\nD) GLOBAL STATS:');
  console.log('-'.repeat(50));
  console.log(`  Total shoe images stored: ${stats.totalImagesStored}`);
...
export async function getShoeSeederReport(): Promise<ShoeSeedReport> {
  const families = await db.select().from(shoeFamilies);
  
  const imageCounts = await db
    .select({
      familyId: shoeImages.familyId,
      count: count(),
    })
    .from(shoeImages)
    .groupBy(shoeImages.familyId);
  ...
  const lockedFamilies = families
    ...
    .map(f => ({ brand: f.brand, family: f.family, imageCount: countMap.get(f.id) || 0 }));
  ...
  const hardFamilies = families
    ...
    .map(f => ({ brand: f.brand, family: f.family, imageCount: countMap.get(f.id) || 0, listingsScanned: f.listingsScanned }));

  const processedItemsResult = await db.execute(sql`SELECT COUNT(*) as count FROM processed_shoe_items`);
