...
export async function storeShoeImage(
  buffer: Buffer,
  brand: string,
  family: string,
  familyId: number,
  ...
  const storagePath = `shoes/${safeBrand}/${safeFamily}/${familyId}/${sha256}.jpg`;
...
export async function getStoredShoeImageUrl(storagePath: string): Promise<string> {
  const bucketName = getBucketName();
